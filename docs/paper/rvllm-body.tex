...
7{,}964.0~tok/s at $N\!=\!64$ (\textbf{1.00$\times$ vLLM}), and
13{,}148.3~tok/s at $N\!=\!128$ (\textbf{0.95$\times$ vLLM}), while remaining
materially behind at $N\!=\!1$ (127.9 vs.\ 165.5~tok/s, \textbf{0.77$\times$}).
The most important changes in this revision are architectural rather than cosmetic:
batch-1 normal decode now defaults to the correct cuBLAS GEMV path, and batched
...
$T\!=\!1$ batch-1 decode, the standard path now defaults to
\texttt{CublasGemvDecode}. For $T\!\ge\!2$, the normal path is an explicit hybrid
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}rrrr@{}}
    ...
    \midrule
    1   & 165.5 & 127.9 & 0.77$\times$ \\
    \rowcolor{gray!8}
    32  & 4{,}467.7 & 4{,}407.5 & 0.99$\times$ \\
    64  & 7{,}972.1 & 7{,}964.0 & 1.00$\times$ \\
    \textbf{128} & \textbf{13{,}903.5} & \textbf{13{,}148.3} & \textbf{0.95$\times$} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}
  \item \textbf{Batch-1 default path selection was wrong.} Normal $T\!=\!1$ decode
        still favored the older fused decode path instead of the better
        \texttt{CublasGemvDecode} path. Fixing that lifted the current normal
        batch-1 path to 127.9~tok/s.
...
\begin{enumerate}
  ...
  \item \textbf{Correct path selection}: batch-1 normal decode now takes the faster
        cuBLAS GEMV route instead of the older fused default.
