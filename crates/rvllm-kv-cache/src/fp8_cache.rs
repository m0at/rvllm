...
impl FP8CacheEngine {
    ...
    /// Dequantize an entire block's key and value data from FP8 back to f16.
    ///
    /// Returns `(key_f16, value_f16)` each of length `block_size * num_heads * head_dim`.
    /// This is used before attention reads from the cache -- the attention kernel
    /// expects f16 inputs. A fused FP8 attention kernel can replace this later.
    ...
    /// Dequantize multiple blocks for a layer, returning contiguous f16 tensors
    /// suitable for passing to the attention kernel.
    ///
    /// `block_indices` specifies which blocks to dequantize.
    /// Returns `(keys, values)` where each is a `Vec<Vec<f16>>` indexed by
    /// position in `block_indices`.
...
mod tests {
    ...
    #[test]
    fn fp8_engine_reshape_and_dequantize_block() {
        ...
        let mut engine = FP8CacheEngine::new(
            num_layers, num_heads, head_dim, block_size,
            num_gpu_blocks, 2, &alloc,
        ).unwrap();

        let hs = num_heads * head_dim;

        // Write 2 tokens into block 1 (slots 2, 3)
        let key: Vec<f16> = (0..2 * hs).map(|i| f16::from_f32((i as f32) * 0.5)).collect();
        let val: Vec<f16> = (0..2 * hs).map(|i| f16::from_f32((i as f32 + 10.0) * 0.5)).collect();
        let slots = vec![2i32, 3];

        engine.reshape_and_cache_fp8(&key, &val, 0, &slots).unwrap();

        // Dequantize block 1 and verify approximate round-trip
        let (key_f16, val_f16) = engine.dequantize_block(0, 1).unwrap();
        assert_eq!(key_f16.len(), block_size * hs);

        for (i, &orig) in key.iter().enumerate() {
            let deq = key_f16[i].to_f32();
            let err = (orig.to_f32() - deq).abs();
            assert!(
                err < 1.0,
                "key[{i}]: orig={}, deq={deq}, err={err}",
                orig.to_f32()
            );
        }
        for (i, &orig) in val.iter().enumerate() {
            let deq = val_f16[i].to_f32();
            let err = (orig.to_f32() - deq).abs();
            assert!(
                err < 1.5,
                "val[{i}]: orig={}, deq={deq}, err={err}",
                orig.to_f32()
            );
        }
    }
    ...
    #[test]
    fn fp8_engine_dequantize_blocks_multi() {
        use rvllm_gpu::prelude::MockGpuAllocator;

        let alloc = MockGpuAllocator::new(1 << 30);
        let mut engine = FP8CacheEngine::new(1, 2, 4, 2, 4, 2, &alloc).unwrap();

        let hs = 2 * 4;
        let key: Vec<f16> = (0..2 * hs).map(|i| f16::from_f32(i as f32)).collect();
        let val: Vec<f16> = (0..2 * hs).map(|i| f16::from_f32(i as f32 + 50.0)).collect();

        // Write to block 0 (slots 0,1) and block 2 (slots 4,5)
        engine.reshape_and_cache_fp8(&key, &val, 0, &[0, 1]).unwrap();
        engine.reshape_and_cache_fp8(&key, &val, 0, &[4, 5]).unwrap();

        let (keys, vals) = engine.dequantize_blocks(0, &[0, 2]).unwrap();
        assert_eq!(keys.len(), 2);
        assert_eq!(vals.len(), 2);
        assert_eq!(keys[0].len(), 2 * hs);
        assert_eq!(keys[1].len(), 2 * hs);
    }

    #[test]
    fn fp8_engine_dequantize_block_oob() {
        use rvllm_gpu::prelude::MockGpuAllocator;

        let alloc = MockGpuAllocator::new(1 << 30);
        let engine = FP8CacheEngine::new(1, 2, 4, 2, 4, 2, &alloc).unwrap();
        assert!(engine.dequantize_block(0, 10).is_err());
    }
}
