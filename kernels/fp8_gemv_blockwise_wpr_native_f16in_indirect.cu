// Indirect-expert variant of the single-output FP8 GEMV — used for
// the per-expert `down_proj` in Qwen 3.6 MoE. Same `expert_idx_ptr +
// base + stride` indirection as the dual_silu_indirect sibling so
// the entire MoE forward becomes graph-captureable.
//
...
extern "C"
__global__ void fp8_gemv_blockwise_wpr_native_f16in_indirect_kernel(
    __half* __restrict__       output,
    const unsigned char* __restrict__ base_w,
    const float* __restrict__  base_s,
    const __half* __restrict__ input,
    const int* __restrict__    expert_idx_ptr,
    long long w_stride,
    long long s_stride,
    int M, int N, int K,
    int num_col_blocks
