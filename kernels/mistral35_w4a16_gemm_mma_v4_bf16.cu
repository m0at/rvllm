// =============================================================
//  mistral35_w4a16_gemm_mma_v4_bf16.cu
// =============================================================
//
// Codex #1 step 5 — BM=128 variant. Doubles the M-tile count per
// CTA from v3 (4 → 8 m16n8k16 mma issues per warp per K-iter)
// while keeping BN=32 / 4 warps / smem A-tile reuse. CTA count
// drops 2× vs v3, A-tile DRAM amortisation doubles, register
// pressure goes from 16 f32/lane to 32 f32/lane (well below the
// sm_121 255-reg/lane limit).
//
// Layout:
//   Grid:  (N/32, ceil(M/128), 1)
//   Block: (128, 1, 1) — 4 warps
//   Smem:  4096 B  (128 × 16 BF16 A tile)
//
// Math, on-disk layout, lane / fragment mapping all identical to
// v1/v2/v3 — only M_TILES grows from 4 to 8.
...
mistral35_w4a16_gemm_mma_v4_bf16_kernel(
    ...
    int K)
{
    ...
    float d4[4]={0,0,0,0}, d5[4]={0,0,0,0}, d6[4]={0,0,0,0}, d7[4]={0,0,0,0};
    ...
    for (int k_outer = 0; k_outer < K; k_outer += BK) {
        // ── A-tile load [128, 16] ────────────────────────────
        // 128 threads × 8 u32 each. Layout u32 grid [BM=128, 8]
        // = 1024 cells. Each thread loads M_TILES rows of one
        // u32 column.
        {
