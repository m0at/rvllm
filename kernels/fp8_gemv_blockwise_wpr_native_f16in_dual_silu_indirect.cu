// Indirect-expert variant of fp8_gemv_dual_silu: reads the expert
// index from a device pointer and computes the weight/scale offsets
// internally. The point is that ALL launch parameters are now stable
// across calls (pointers come from arena regions; strides are
// per-MoE-block constants), which makes the per-expert MoE chain
// CUDA-Graph-captureable: capture once, replay per token.
//
// Same epilogue as dual_silu — one f16 output silu(g)*u, no
// gate_region / up_region scratch.
//
// Layout reminder:
//   experts_gate_proj_fused weight: [num_experts, N, K] fp8, row-major
//   experts_gate_proj_fused scale : [num_experts, N/128, K/128] f32
// Per-expert byte strides:
//   w_stride_bytes = N * K
//   s_stride_bytes = (N/128) * (K/128) * 4
//
// Phase 4b-prep iter34.
...
extern "C"
__global__ void fp8_gemv_blockwise_wpr_native_f16in_dual_silu_indirect_kernel(
    ...
    const int* __restrict__    expert_idx_ptr,  // *e
    long long w_stride,                         // bytes between expert weight rows
    long long s_stride,                         // f32 elements between scale rows
    int M, int N, int K,
    int num_col_blocks
) {
