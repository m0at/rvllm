...
// flash_attention_split_decode_nvfp4kv.cu. Phase-1 split kernels
// write f32 tmp_out (cycle-21 fix) and are dtype-agnostic at the
// activation boundary — only their symbols are renamed for parallel
// dispatch. The Phase-2 reducer (paged_attention_reduce_bf16_kernel)
// is the one whose `output` parameter and the four output-write
// sites flip f16 → bf16.
//
...
// ---------------------------------------------------------------------
extern "C"
__global__ void flash_attention_2_decode_nvfp4kv_split_bf16out_kernel(
...
extern "C"
__global__ void flash_attention_2_decode_nvfp4kv_split_bc16_bf16out_kernel(
    // tmp_out is f32 [S,H,P,D] — matches the BC=32 split kernel and
    // the f32 contract `paged_attention_reduce_bf16_kernel` expects.
...
extern "C"
__global__ void paged_attention_reduce_bf16_kernel(
    __nv_bfloat16* __restrict__ output,         // [S, H, D]
    ...
    if (num_partitions == 1) {
        for (int d = tid; d < head_dim; d += REDUCE_THREADS) {
            output[out_base + d] = __float2bfloat16(tmp_out[tmp_out_base + d]);
        }
        return;
    }
