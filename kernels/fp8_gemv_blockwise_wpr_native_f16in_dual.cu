// Dual-output FP8 GEMV: same input, two weight matrices, two
// outputs. Fuses the gate and up projections of Qwen 3.6's MoE
// experts (and shared expert) into one launch — 8 routed experts
// × 30 MoE layers / token saves 240 launches/token plus halves
// the input-tile bandwidth (each `x_row` chunk now feeds two
// matmuls instead of one).
//
// Math is bit-identical to two separate
// `fp8_gemv_blockwise_wpr_native_f16in_kernel` calls run on the
// same input — see that kernel for the per-chunk derivation.
//
// Launch config: grid (ceil(N/8), M), block 256. Same as the
// single-output sibling. M=1 in our hot path; N is gate/up's
// shared output dim (= n_int = 512 in Qwen 3.6 35B-A3B).
//
// Phase 4b-prep iter31.

#include <cuda_fp16.h>

// Inlined helpers (mirroring fp8_gemv.cu — PTX is per-file).
...
extern "C"
__global__ void fp8_gemv_blockwise_wpr_native_f16in_dual_kernel(
    __half* __restrict__       out_g,        // [M, N] f16 (gate)
    __half* __restrict__       out_u,        // [M, N] f16 (up)
    const unsigned char* __restrict__ w_g,   // [N, K] fp8 (gate weight)
    const unsigned char* __restrict__ w_u,   // [N, K] fp8 (up weight)
    const float* __restrict__  s_g,          // [num_row_blocks, num_col_blocks] f32
    const float* __restrict__  s_u,          // [num_row_blocks, num_col_blocks] f32
    ...
    // Remainder (matches the single-output kernel).
    {
        int aligned_k = (K / 8) * 8;
    ...
    if (lane == 0) {
        ...
        out_g[off] = __float2half(acc_g);
        out_u[off] = __float2half(acc_u);
    }
}
