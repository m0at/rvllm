...
//
// Uses the "halved" rotation convention (standard for Qwen3.5 / HuggingFace):
//   Pairs dimensions (i, i + rotary_dim/2) -- first-half / second-half pairs.
//   cos/sin cache has rotary_dim/2 entries per position.
//
...
//   Shared memory: none
//
// Each thread handles one pair of dimensions (i, i + half_rotary) for one head of one token.
