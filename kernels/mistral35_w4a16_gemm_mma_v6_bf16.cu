// =============================================================
//  mistral35_w4a16_gemm_mma_v6_bf16.cu
// =============================================================
//
...
__device__ __forceinline__ uint32_t pack_two_bf16_v6(
...
mistral35_w4a16_gemm_mma_v6_bf16_kernel(
    ...
    int K)
{
    ...
    for (int k_iter = 0; k_iter < K_iters; ++k_iter) {
        ...
        // Issue prefetch for stage (k+1) if there is one.
        if (k_iter + 1 < K_iters) {
            issue_stage_load(next_stage, k_outer + BK);
            cp_async_commit();
        }

        // Wait for THIS iteration's stage. After this, there is
        // either 0 pending (last iter) or 1 pending (prefetch).
        if (k_iter + 1 < K_iters) {
        ...
        for (int m_local = 0; m_local < M_TILES; ++m_local) {
            ...
            uint32_t a[4];
            {
                ...
                a[0] = pack_two_bf16_v6(v0, v1);
                a[1] = pack_two_bf16_v6(v2, v3);
                a[2] = pack_two_bf16_v6(v4, v5);
                a[3] = pack_two_bf16_v6(v6, v7);
            }
            asm volatile(
