...
// ============================================================================
// FP8 E4M3 paged-prefill variant (BC=32) — multi-query self-attention
// with causal mask. Used by the Sm121 FA2-PTX batch-prefill path.
// ============================================================================
//
// Structured from `flash_attention_2_decode_fp8kv_kernel` (FP8 loads +
// online softmax) and `flash_attention_2_kernel` (q_len loop + causal
// mask). Each block handles one (seq, head) pair and iterates over the
// q_len query tokens of that seq. `cu_seqlens_q` is the standard
// `[num_seqs+1]` prefix-sum array used upstream — the spec is
// identical to the FA3 SM90 ABI except we launch the PTX kernel
// directly from the Rust launcher.
//
// Launch config (see PagedPrefillFp8Launcher::launch):
//   Grid  (num_seqs, num_heads, 1)
//   Block (FA2_THREADS=128, 1, 1)
//   Smem  = 2 * FA2_BC * head_dim * 4 + FA2_BC * 4 + warps*4
//
// head_dim>256 spills the BC=32 smem budget on sm_121 — the BC=16
// variant immediately below handles that shape (Gemma 4 global-attn).

extern "C"
__global__ void flash_attention_2_prefill_fp8kv_kernel(
    __half* __restrict__ output,                 // [num_q_tokens, num_heads, head_dim] f16
    const unsigned char* __restrict__ query,     // [num_q_tokens, num_heads, head_dim] fp8
    const unsigned char* __restrict__ key_cache, // [num_blocks, block_size, num_kv_heads, head_dim]
    const unsigned char* __restrict__ value_cache,
    const int* __restrict__ block_tables,        // [num_seqs, max_blocks_per_seq]
    const int* __restrict__ context_lens,        // [num_seqs]
    const int* __restrict__ cu_seqlens_q,        // [num_seqs+1] prefix sum
    const float* __restrict__ q_descale,         // scalar
    const float* __restrict__ k_descale,         // scalar
    const float* __restrict__ v_descale,         // scalar
    float scale,
    int num_heads,
    int num_kv_heads,
    int head_dim,
    ...
    // Per query token in this seq.
    for (int qi = 0; qi < q_len; qi++) {
        const int q_pos_global = q_start + qi;

        // Load Q row (FP8 -> f32 with descale + attention scale folded in).
        float q_reg[8];
        for (int r = 0; r < dims_per_thread && r < 8; r++) {
            int d = tid + r * FA2_THREADS;
            if (d < head_dim) {
...
// mirrors the BC=16 decode kernel's smem-budget story.
extern "C"
__global__ void flash_attention_2_prefill_fp8kv_bc16_kernel(
    __half* __restrict__ output,
    const unsigned char* __restrict__ query,
    const unsigned char* __restrict__ key_cache,
    const unsigned char* __restrict__ value_cache,
