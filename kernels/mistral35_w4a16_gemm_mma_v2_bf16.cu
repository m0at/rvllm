// =============================================================
//  mistral35_w4a16_gemm_mma_v2_bf16.cu
// =============================================================
//
// Codex #1 step 3 — multi-warp variant of the tensor-core W4A16
// GEMM. Same on-disk NVFP4 layout, same `mma.sync.aligned.m16n8k16.row.col.f32.bf16.bf16.f32`
// instruction, but the CTA now staggers 4 warps over BN=32 cols of
// the output tile and shares the BM×BK activation slice through
// shared memory. Each K-block:
//
//   (1) All 128 threads cooperatively load A[BM=16, BK=16] from
//       DRAM into smem (one u32 = 2 bf16 per thread).
//   (2) Each warp packs its own A fragment by reading the smem
//       tile through `ldmatrix`-equivalent scalar loads.
//   (3) Each warp dequants its B fragment (4 nibbles → 4 bf16)
//       for its own [BK=16, 8] N-slice.
//   (4) One mma per warp.
//
// Compared to the v1 single-warp kernel:
//   * BN per CTA = 32 (4 warps × 8 cols) instead of 8.
//   * A is read from DRAM once per CTA per K-block, not once per
//     warp — 4× fewer A reads on the inner loop.
//   * CTA count drops from (N/8 × M/16) to (N/32 × M/16).
//
// Math is identical to v1: out[m,n] = Σ_k act[m,k] · w[n,k]
// with w = fp4_decode(nibble) · scale_e4m3 · alpha.
...
mistral35_w4a16_gemm_mma_v2_bf16_kernel(
    ...
    int K)
{
    ...
    // 16 × 16 BF16 A tile = 256 BF16 = 512 bytes per CTA. Pad K
    // to 16 + 2 to avoid bank conflicts on the column-strided read
    // pattern used by the warps to pack their A fragments. Wait —
    // for BK=16 and BF16, two rows fit in 16 bytes (4 banks), so no
    // padding strictly needed at this size. Keep it simple.
    ...
    for (int k_outer = 0; k_outer < K; k_outer += BK) {
        // ── (1) Cooperative A-tile load ──────────────────────
        // 128 threads × 1 u32 = 256 bf16. Lay out u32 grid as
        // [BM=16, BK/2=8], 16 × 8 = 128 u32 — one per thread.
        {
            const int row = tid >> 3;          // 0..15
            const int col_u32 = tid & 7;       // 0..7
            const int k_pos = col_u32 * 2;     // 0,2,...,14
        ...
        // ── (2) Pack per-warp A fragment from smem ───────────
        // Standard m16n8k16 A layout (row.col, row-major source).
        const int c_lo = tg * 2;
        const int c_hi = c_lo + 8;
        uint32_t a[4];
        {
            // gid (0..7) selects row_lo ∈ [0,7]; row_hi = row_lo + 8.
            const __nv_bfloat16 v0 = smem_a[gid    ][c_lo    ];
            const __nv_bfloat16 v1 = smem_a[gid    ][c_lo + 1];
            const __nv_bfloat16 v2 = smem_a[gid + 8][c_lo    ];
            const __nv_bfloat16 v3 = smem_a[gid + 8][c_lo + 1];
            const __nv_bfloat16 v4 = smem_a[gid    ][c_hi    ];
            const __nv_bfloat16 v5 = smem_a[gid    ][c_hi + 1];
            const __nv_bfloat16 v6 = smem_a[gid + 8][c_hi    ];
            const __nv_bfloat16 v7 = smem_a[gid + 8][c_hi + 1];
        ...
        // Same per-lane mapping as v1. Each warp's `n_warp_base`
        // shifts the N slice by warp_id × 8 cols.
        uint32_t b[2];
        if (n_col < N) {
