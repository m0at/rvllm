// Paged attention kernels for SM86 (Ampere).
//
...
//
// Build:
//   nvcc -shared -o libfa_sm86_kernels.so paged_attention_sm86.cu \
...
// -------------------------------------------------------------------
// C ABI wrappers. Symbol names: fa_sm86_*
// -------------------------------------------------------------------

extern "C" {

int fa_sm86_workspace_size(int batch_size, int num_heads, int max_num_splits) {
    (void)batch_size; (void)num_heads; (void)max_num_splits;
    return 0; // single-split, no workspace needed
}

int fa_sm86_paged_decode(
