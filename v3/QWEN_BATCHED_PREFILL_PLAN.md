...
| **3b** | Wire the cuBLASLt `fp8_gemm` path inside the dispatcher's m≥2 branch. Adds: a small `fp8_quantize_per_token_f16` kernel (~50 LOC CU) so the input gets quantised + per-token f32 amax in one launch, and either a direct pass-through of the `[N/128, K/128]` row-major weight blockscale or a Gemma-style transpose into MN-major depending on what cuBLASLt accepts at runtime. Per-shape cosine vs the looped-m=1-GEMV reference, ≥0.9999 across q/k/v/o + gate/up/down dims. | ~450 | `kernels/fp8_quantize.cu`, `qwen36_bring_up.rs` | Extended `probe-qwen-fp8-proj` (loop m=1 N times → reference; dispatcher m=N → candidate; cosine ≥ 0.9999) |
