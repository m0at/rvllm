...
| **3c** | sm_121-specific perf: replace the looped-GEMV fallback in 3b with the existing CUTLASS SM120 blockwise FP8 GEMM (`kernels/cutlass_fp8_gemm_blockscale_sm120.cu`, already used by Gemma at lm_head). Hard-asserts `M ≥ 128`, so dispatch becomes: m=1 → GEMV, m∈[2,127] → looped GEMV (still no fast path; that needs Phase 5 batched linear-attn first), m≥128 → CUTLASS SM120. Phase 4 only flips the caller to m=N=prompt_len, so the m≥128 case fires for any prompt with ≥128 tokens (the realistic case). | ~200 | `qwen36_bring_up.rs` | Same probe extended with m∈{128, 256, 1024} |
